...
def create_gl_code(body: GlCodeCreate, user: dict = Depends(require_role("admin"))):
...
def update_gl_code(gl_code_id: str, body: GlCodeUpdate, user: dict = Depends(require_role("admin"))):
...
def delete_gl_code(gl_code_id: str, user: dict = Depends(require_role("admin"))):
    with engine.begin() as conn:
        conn.execute(
...
def create_company(body: CompanyCreate, user: dict = Depends(require_role("admin"))):
    with engine.begin() as conn:
        row = conn.execute(
...
def update_company(company_id: str, body: CompanyUpdate, user: dict = Depends(require_role("admin"))):
    if not body.name:
        return {"updated": False}
...
def delete_company(company_id: str, user: dict = Depends(require_role("admin"))):
    with engine.begin() as conn:
        conn.execute(
...
def create_expense_type(body: ExpenseTypeCreate, user: dict = Depends(require_role("admin"))):
    with engine.begin() as conn:
        row = conn.execute(
...
def update_expense_type(expense_type_id: str, body: ExpenseTypeUpdate, user: dict = Depends(require_role("admin"))):
    if not body.name:
        return {"updated": False}
...
def delete_expense_type(expense_type_id: str, user: dict = Depends(require_role("admin"))):
    with engine.begin() as conn:
        conn.execute(
...
def create_card_account(body: CardAccountCreate, user: dict = Depends(require_role("admin"))):
    with engine.begin() as conn:
        row = conn.execute(
...
def update_card_account(account_id: str, body: CardAccountUpdate, user: dict = Depends(require_role("admin"))):
    fields = {}
...
def delete_card_account(account_id: str, user: dict = Depends(require_role("admin"))):
    with engine.begin() as conn:
        result = conn.execute(
...
def create_vendor_mapping(body: VendorMappingCreate, user: dict = Depends(require_role("admin"))):
    with engine.begin() as conn:
        row = conn.execute(
...
def update_vendor_mapping(mapping_id: str, body: VendorMappingUpdate, user: dict = Depends(require_role("admin"))):
    fields = {}
...
def delete_vendor_mapping(mapping_id: str, user: dict = Depends(require_role("admin"))):
    with engine.begin() as conn:
        result = conn.execute(
...
def create_city_company_rule(body: CityCompanyRuleCreate, user: dict = Depends(require_role("admin"))):
    with engine.begin() as conn:
        row = conn.execute(
...
def update_city_company_rule(rule_id: str, body: CityCompanyRuleUpdate, user: dict = Depends(require_role("admin"))):
    fields = {}
...
def delete_city_company_rule(rule_id: str, user: dict = Depends(require_role("admin"))):
    with engine.begin() as conn:
        result = conn.execute(
